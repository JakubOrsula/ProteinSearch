...
def run_search(pdb_id: str, num_results: int, radius: float, schema: str):
...
def stress(ids: List[str], num_results: int, radius: float, workers: int):
    with open('response_schema.json') as f:
        schema = json.load(f)

    executor = ThreadPoolExecutor(workers)

    run_search2 = functools.partial(run_search, num_results=num_results, radius=radius, schema=schema)
    jobs = [executor.submit(run_search2, data) for data in ids]
    results = []
...
if __name__ == '__main__':
    ...
    stress(pdb_ids, num_results=args.results, radius=args.radius, workers=args.workers)
